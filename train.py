...
def train_model(
    ...
    train_dataset = BasicDataset(os.path.join(dir_img, 'train'), 
                                 os.path.join(dir_mask, 'train'), 
                                 img_scale)
    
    val_dataset = BasicDataset(os.path.join(dir_img, 'val'), 
                               os.path.join(dir_mask, 'val'), 
                               img_scale)
            ...
            for batch in train_loader:
                ...
                with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=amp):
                    ...
                    if model.n_classes == 1:
                        ...
                        loss += dice_loss(torch.sigmoid(masks_pred.squeeze(1)), true_masks.squeeze(1).float(), multiclass=False)
                    else:
